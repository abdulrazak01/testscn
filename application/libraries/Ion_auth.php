...
class Ion_auth
{
	...
	public function __construct()
	{
		...
		if($this->logged_in()){
            $user 		= $this->user()->row();
            $group 		= $this->ion_auth_model->get_users_groups($user->id)->row();
            $username 	= $this->ion_auth_model->get_name_user($user->id)->row();
            $profil 	= $this->ion_auth_model->get_image_profil($user->id)->row();

            define("ID_GROUP", $group->id);
            define("GROUP_NAME", $group->group);
            define("DESCRIPTION", $group->description);
            define("ID_ASSIGNED_OPT", $group->id_assigned_opt);
            define("IPQC", $group->function);

            define("USER_ID", $user->id);

            define("USER_NAME", $username->first_name);
            define("EMPLOYEE_NO", $username->employee_no);

            define("PROFIL", $profil->file_picture);
           
            // log_r($group->function); 
        }
	}
